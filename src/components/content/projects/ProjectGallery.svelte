...
<div class="mdc-layout-grid__cell mdc-layout-grid__cell--span-12" style="--jdev-gallery-animation-duration: {animationParams.content.duration}ms;">
  <FlyingHeadline localeKey="project.gallery" transitionDuration={animationParams.headline.duration} transitionDelay={animationParams.headline.delay} />
  <ImageList class="jdev-project-image-list">
    {#each projectData.projectPage.gallery as item, i (i)}
      <ImageListItem>
        <ImageAspectContainer>
          <ImageListImage
            src={item.thumb}
            tag="div"
            style="animation-delay: {animationParams.content.delay + animationParams.content.iterationDelay * i}ms; background-size: contain; background-image: url({item.thumb});"
            on:click={() => {
              openGallery(i)
            }}
          />
