...
<div class="mdc-layout-grid mdc-typography--body1 jdev-route-resume" in:svelteTransitionFade={{ duration: routingFadeDuration }}>
  ...
  <div class="mdc-layout-grid__inner">
    {#each resumeEducationList as item, i (i)}
      <ResumeItem {item} delay={i * atomTransitionDuration + headerTransitionDuration} transitionDuration={atomTransitionDuration} />
    {/each}
  </div>
  <FlyingHeadline localeKey="resume.experience.headline" />
  <div class="mdc-layout-grid__inner">
    {#each resumeExperienceList as item, i (i)}
      <ResumeItem {item} delay={i * atomTransitionDuration + headerTransitionDuration} transitionDuration={atomTransitionDuration} />
    {/each}
  </div>
  <FlyingHeadline localeKey="resume.skills.headline" />
  <div class="mdc-layout-grid__inner">
    {#each resumeSkillList as item, i (i)}
      <ResumeItem {item} delay={i * atomTransitionDuration + headerTransitionDuration} transitionDuration={atomTransitionDuration} />
    {/each}
  </div>
</div>
